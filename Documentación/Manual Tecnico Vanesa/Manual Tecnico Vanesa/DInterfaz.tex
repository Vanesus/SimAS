...
\chapter{Diseño de la interfaz}
 ...
 En este capítulo se abordará el diseño de cada uno de los componentes que forman la interfaz de la aplicación. Puesto que muchos componentes son similares, se mostrará únicamente un componente de cada tipo. Cabe destacar que este análisis será somero debido a que en el \textit{Manual de Usuario} se realiza un análisis más exhaustivo de cada componente de la interfaz.

A continuación se muestran las capturas de la interfaz final de la aplicación con una explicación de cada una de las partes que la componen. Cabe destacar que los informes de la gramática y de la simulación no se reflejan aquí, puesto que son ficheros pdf que se guardan directamente al disco y no se visualizan desde
la aplicación. Estos informes de muestran con más detalle en el \textit{Anexo B: Informes}.
...
%
%
%La \textbf{ventana de ayuda} contiene la ayuda de la aplicación.
%Esta se ha construido en formato HTML simple (sin css) y con menús navegables.
%Esto hace que no sea preciso utilizar más componentes en la ventana
%(que solo haría que el usuario viera más complicado el hecho
%de consultar la ayuda). Además, como ya se ha mencionado,
%el usuario puede consultar directamente el contenido de ayuda
%pulsando sobre el botón de ayuda que encontrará en cada ventana (lo que
%mostrará directamente la ayuda, sin ser preciso de ninguna otra operación
%adicional). Para saber más sobre
...
%La ventana está compuesta de los siguientes elementos:
%\begin{enumerate}
% \item \textbf{Contenido de la ayuda}: muestra una página html de la ayuda.
%Cada página posee un menú de navegación que permite al usuario cambiar de
%capítulo.
%
%\end{enumerate}
%
...
%
%A continuación, y tras haber analizado los componentes que forman la interfaz
%de SimAS, se muestra el \textbf{diagrama de la arquitectura} de la interfaz. Nótese que este
%diagrama está inspirado en la especificación de los casos de uso que ya se realizó
%durante la fase de análisis y muestra todas las acciones a llevar a cabo en la interfaz
%(las acciones de edición de gramáticas se encuentran agrupadas bajo el concepto de
%\textbf{editar gramática}). Además, nótese que no se muestra ningún elemento
%relativo al tutorial, puesto que es un documento pdf que se adjunta con la aplicación
%(permitiéndose la mo\-di\-fi\-ca\-ción de este en un futuro sin ser preciso realizar una mo\-di\-fi\-ca\-ción
%del programa).
%
%  \begin{figure}[H]
%      \begin{center} 
